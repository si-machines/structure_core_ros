...
#include <structure_core/StructureDriverConfig.h>
...
private:
    ...
    {
        if(not f.isValid() or rgb_image_pub_.getNumSubscribers() == 0)
        {
            return;
        }
        std::string rgb_frame_id = "camera_rgb_optical_frame";
        rgb_image_pub_.publish(imageFromVisibleFrame(rgb_frame_id, f));
        rgb_info_pub_.publish(infoFromFrame(rgb_frame_id, f));
    }
...
public:
    ...
    SessionDelegate(ros::NodeHandle& n, ros::NodeHandle& pnh)
    {
        ...
        depth_image_pub_ = dn.advertise<sensor_msgs::Image>("image_raw", 10);
        ...
        ros::NodeHandle vn(n, "rgb");
        rgb_image_pub_ = vn.advertise<sensor_msgs::Image>("image_raw", 10);
        rgb_info_pub_ = vn.advertise<sensor_msgs::CameraInfo>("camera_info", 10);
...
void fillCaptureSettingsFromDynamicReconfigureConfig(structure_core::StructureDriverConfig& config, ST::CaptureSessionSettings& settings) {
    settings.frameSyncEnabled = config.frameSyncEnabled;
    ...
    settings.lowLatencyIMU = config.lowLatencyIMU;
    /** @brief Set to true to apply a correction filter to the depth before streaming. This may effect performance. */
    settings.applyExpensiveCorrection = config.applyExpensiveCorrection;
    /** @brief Set to true to enable depth streaming. */
    settings.structureCore.depthEnabled = config.depthEnabled;
    /** @brief Set to true to enable infrared streaming. */
    settings.structureCore.infraredEnabled = config.infraredEnabled;
    /** @brief Set to true to enable visible streaming. */
    settings.structureCore.visibleEnabled = config.visibleEnabled;
    /** @brief Set to true to enable accelerometer streaming. */
    settings.structureCore.accelerometerEnabled = config.accelerometerEnabled;
    /** @brief Set to true to enable gyroscope streaming. */
    settings.structureCore.gyroscopeEnabled = config.gyroscopeEnabled;
    ...
    settings.structureCore.visibleApplyGammaCorrection = config.visibleApplyGammaCorrection;
    /** @brief Enable auto-exposure for infrared frames. */
    settings.structureCore.infraredAutoExposureEnabled = config.infraredAutoExposureEnabled;
    ...
    settings.structureCore.depthFramerate    = config.depthFramerate;
    ...
    settings.structureCore.disableInfraredIntensityBalance = config.disableInfraredIntensityBalance;
...
void fillDynamicReconfigureConfigFromCaptureSettings(ST::CaptureSessionSettings& settings, structure_core::StructureDriverConfig& config) {
    config.frameSyncEnabled = settings.frameSyncEnabled;
    config.lowLatencyIMU = settings.lowLatencyIMU;
    config.applyExpensiveCorrection = settings.applyExpensiveCorrection;
    config.depthEnabled = settings.structureCore.depthEnabled;
    config.infraredEnabled = settings.structureCore.infraredEnabled;
    config.visibleEnabled = settings.structureCore.visibleEnabled;
    config.accelerometerEnabled = settings.structureCore.accelerometerEnabled;
    config.gyroscopeEnabled = settings.structureCore.gyroscopeEnabled;
    config.visibleApplyGammaCorrection = settings.structureCore.visibleApplyGammaCorrection;
    config.infraredAutoExposureEnabled = settings.structureCore.infraredAutoExposureEnabled;
    config.disableInfraredIntensityBalance = settings.structureCore.disableInfraredIntensityBalance;
    config.depthFramerate = settings.structureCore.depthFramerate;
}
...
private:
    ...
    dynamic_reconfigure::Server<structure_core::StructureDriverConfig> server;

public:
    StructureDriver(): n(), pnh("~"), settings(), session(), delegate(n, pnh), server()
    {
        dynamic_reconfigure::Server<structure_core::StructureDriverConfig>::CallbackType f =
            boost::bind(&StructureDriver::configCallback, this, _1, _2);
        server.setCallback(f);

        loadDefaultSettings(settings);

        // now put the default settings into a config
        // note: we really don't care what the defaults are in rqt_reconfigure
        // (we have our own defaults) so skip this line
        //server.getConfigDefault(config);

        structure_core::StructureDriverConfig config;
        fillDynamicReconfigureConfigFromCaptureSettings(settings, config);

        // push back the true default values to the GUI
        server.updateConfig(config);
    }

    void loadDefaultSettings(ST::CaptureSessionSettings& settings) {
        settings.source = ST::CaptureSessionSourceId::StructureCore;
        // TODO: load default settings from ROS parameters
    ...
    void configCallback(structure_core::StructureDriverConfig &config, uint32_t level) {
        ROS_INFO("Driver configuration updated");
        fillCaptureSettingsFromDynamicReconfigureConfig(config, settings);
        restartStreaming();
    }
    ...
    void startStreaming() {
        ...
        ROS_INFO_STREAM(settings.structureCore.initialInfraredGain);
